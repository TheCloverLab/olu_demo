...
export default function CreatorConsole() {
  ...
  if (loading) {
    return <div className="max-w-6xl mx-auto px-4 py-8 text-[var(--olu-text-secondary)]">Loading creator console...</div>
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 pb-24 md:pb-8 space-y-6">
      ...
      {tab === 'campaigns' && (
        <div className="grid lg:grid-cols-[1.2fr,0.95fr] gap-4">
          <div className="rounded-[32px] p-6 space-y-5 border border-[var(--olu-card-border)] bg-[#091523] shadow-[0_16px_50px_rgba(2,8,23,0.22)]">
            ...
            <div className="rounded-[28px] bg-[var(--olu-card-bg)] p-5 border border-[var(--olu-card-border)]">
              <div className="flex items-center justify-between gap-4 mb-3">
                <div>
                  <p className="font-bold text-lg">{workflow?.campaign.brand_name || 'No campaign'}</p>
                  <p className="text-[var(--olu-text-secondary)] text-xs mt-1">Promotion request for {myTarget?.creator?.name || user?.name || 'creator'}</p>
                </div>
              ...
              <div className="grid md:grid-cols-3 gap-3 mt-4">
                ...
                <div className="rounded-2xl bg-[var(--olu-header-bg)] p-4 border border-[var(--olu-card-border)]">
                  <p className="text-[var(--olu-text-secondary)] text-xs mb-1">Placement fee</p>
                  <p className="font-semibold text-sm">${myTarget?.offer_amount ?? 0}</p>
                </div>
                <div className="rounded-2xl bg-[var(--olu-header-bg)] p-4 border border-[var(--olu-card-border)]">
                  <p className="text-[var(--olu-text-secondary)] text-xs mb-1">Queue status</p>
          ...
          <div className="rounded-[32px] p-6 border border-[var(--olu-card-border)] bg-[#091523] shadow-[0_16px_50px_rgba(2,8,23,0.22)]">
              ...
              {[
                ...
                <div key={item.title} className="rounded-2xl bg-[var(--olu-card-bg)] p-4 border border-[var(--olu-card-border)] flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[var(--olu-text-secondary)] text-xs mt-1 leading-relaxed">{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="rounded-2xl bg-[var(--olu-card-bg)] p-4 mt-4 border border-[var(--olu-card-border)]">
              <p className="text-[var(--olu-text-secondary)] text-xs mb-1">Creator reward locked</p>
              <p className="font-black text-2xl">${myTarget?.creator_reward ?? 0}</p>
            </div>

            <div className="grid grid-cols-3 gap-3 mt-4">
              <div className="rounded-2xl bg-[var(--olu-card-bg)] p-4 border border-[var(--olu-card-border)]">
                <p className="text-[var(--olu-text-secondary)] text-xs mb-1">Views</p>
            ...
            <div className="space-y-3 mt-4">
              {(workflow?.events || []).slice(0, 3).map((event) => (
                <div key={event.id} className="rounded-2xl bg-[var(--olu-card-bg)] p-4 border border-[var(--olu-card-border)]">
                  <p className="font-semibold text-sm">{event.title}</p>
                  <p className="text-[var(--olu-text-secondary)] text-xs mt-1">{event.detail}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab === 'fans' && (
        <div className="rounded-[32px] p-5 space-y-3 border border-[var(--olu-card-border)] bg-[#091523] shadow-[0_16px_40px_rgba(2,8,23,0.16)]">
          {fans.map((fan) => (
          ...
          <div className="rounded-[32px] p-5 space-y-3 border border-[var(--olu-card-border)] bg-[#091523] shadow-[0_16px_40px_rgba(2,8,23,0.16)]">
            ...
            {licenses.map((item) => (
              <div key={item.id} className="p-4 rounded-[24px] bg-[var(--olu-card-bg)] border border-[var(--olu-card-border)]">
                ...
                <p className="text-[var(--olu-text-secondary)] text-xs">{item.type} · {item.status}</p>
              </div>
            ))}
          ...
          <div className="rounded-[32px] p-5 space-y-3 border border-[var(--olu-card-border)] bg-[#091523] shadow-[0_16px_40px_rgba(2,8,23,0.16)]">
            ...
            {infringements.map((item) => (
              <div key={item.id} className="p-4 rounded-[24px] bg-[var(--olu-card-bg)] border border-[var(--olu-card-border)]">
                ...
                <p className="text-[var(--olu-text-secondary)] text-xs">{item.content} · {item.status}</p>
              </div>
            ))}
            {infringements.length === 0 && <p className="text-[var(--olu-text-secondary)] text-sm">No infringement records.</p>}
          </div>
        </div>
      )}
          ...
          {products.map((product) => (
            <div key={product.id} className="flex items-center justify-between p-4 rounded-[24px] bg-[var(--olu-card-bg)] border border-[var(--olu-card-border)]">
              <div>
                ...
                <p className="text-[var(--olu-text-secondary)] text-xs">{product.status} · stock {product.stock}</p>
              </div>
