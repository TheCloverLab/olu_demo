              ...
              {[
                ...
                <button key={item.type} onClick={() => setStep(2)} className="w-full p-3 glass glass-hover rounded-xl text-left">
                  <div className="flex items-center justify-between mb-0.5">
                    ...
                    <span className="text-violet-400 text-xs font-semibold">{item.price}</span>
                  </div>
      ...
      <div className="px-4 -mt-8 relative">
        <div className="flex items-end justify-between mb-4">
          ...
          <div className="flex gap-2 mb-1">
            {currentRole !== 'creator' && (
              <button onClick={() => setShowIPLicense(true)} className="px-3 py-2 rounded-xl glass glass-hover text-xs font-medium flex items-center gap-1.5 text-violet-300 border border-violet-500/30">
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          {creator.verified && <BadgeCheck size={18} className="text-violet-400" fill="currentColor" />}
        </div>
      ...
      <div className="px-4">
        ...
        {tab === 'members' && (
          <div className="space-y-3">
            {tiers.map((tier) => (
              <div key={tier.id} className={clsx('p-4 glass rounded-xl border', tier.popular ? 'border-violet-500/50' : 'border-olu-border')}>
                {tier.popular && <div className="text-xs text-violet-400 font-semibold mb-2">⭐ Most Popular</div>}
