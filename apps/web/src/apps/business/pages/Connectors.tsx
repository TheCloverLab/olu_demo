...
function BrandIcon({ provider, size = 22 }: { provider: string; size?: number }) {
  ...
  if (Icon) return <Icon size={size} color="white" />
  // Temu & SHEIN don't have Simple Icons yet — use wordmark text
  if (provider === 'Temu') return <span className="text-white font-black text-[10px] tracking-tight">Temu</span>
  if (provider === 'SHEIN') return <span className="text-white font-black text-[8px] tracking-wider">SHEIN</span>
  return <span className="text-white font-bold text-sm">{provider[0]}</span>
}
